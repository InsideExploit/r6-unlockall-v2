...
int main()
{
	...
	while (!sdk::IsKeyPressed(VK_END)) {
		...
		if (sdk::IsKeyPressed(enable)) {
			...
			uint8_t ret[] = { 0xC3 };
			WriteProcessMemory(memory::handle, (LPVOID)(memory::base + 0x20DFFA0), rax, sizeof(rax), NULL);
			WriteProcessMemory(memory::handle, (LPVOID)(memory::base + 0x20DFFAA), ret, sizeof(ret), NULL);
