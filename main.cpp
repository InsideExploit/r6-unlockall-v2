...
int main()
{
	...
	while (!sdk::IsKeyPressed(VK_END)) {
		...
		if (sdk::IsKeyPressed(enable)) {
			...
			uint8_t ret[] = { 0xC3 };
			WriteProcessMemory(memory::handle, (LPVOID)(memory::base + 0x1B0B860), rax, sizeof(rax), NULL);
			WriteProcessMemory(memory::handle, (LPVOID)(memory::base + 0x1B0B860), ret, sizeof(ret), NULL);
